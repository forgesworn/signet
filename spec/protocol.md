...
### Layer 4 — Optional External Bond (Skin in the Game)

Communities may require professional verifiers to maintain an externally
enforceable Bitcoin bond. If they do, the external mechanism defines how funds
are locked, adjudicated, slashed, or returned. Signet records the community's
threshold and the verifier's evidence; it does not custody or move the funds.

An enforceable external bond can be:
...
Bond amount is configurable per community policy. A casual group might require
100,000 sats. A high-security community might require 1,000,000 sats. The
economic deterrent exists only when the community independently verifies that
the required funds are actually locked and slashable.

The core protocol currently carries a BIP-322 address-control attestation with a
declared amount. BIP-322 proves control of the named address and binds the
declaration to the verifier's Nostr key. It does **not** prove the address balance,
lock funds, monitor reserves, or enforce slashing. Clients MUST NOT treat that
attestation alone as an active economic bond. They need independent evidence from
the community's chosen bond mechanism. NWC is a wallet communication protocol;
it neither locks nor escrows funds and has no role in Signet bond verification.

The bond mechanism is optional. Communities that do not require one omit the
bond threshold from their policy (kind 30078).
...
Reporter ──── publishes kind 31000 (type: challenge) ────► challenge attestation
              ...
              If N trusted accounts (Tier 3+) confirm:
                ...
                │
                ├─ External bond provider may slash under its own rules
                │  (Signet records the claimed bond action only)
                │
...
4. Risk losing an independently verified external bond, if the community requires one
