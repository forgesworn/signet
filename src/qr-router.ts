...
import { VALID_AGE_RANGES } from './presentation.js';
...
export const QR_MAX_PAYLOAD_SIZE = 8192;
...
function isValidVerifyRequest(obj: Record<string, unknown>): obj is Record<string, unknown> & VerifyRequest {
  ...
  if (!(VALID_AGE_RANGES as readonly string[]).includes(obj.requiredAgeRange)) return false;
...
function isValidAuthRequest(obj: Record<string, unknown>): obj is Record<string, unknown> & AuthRequest {
  if (typeof obj.requestId !== 'string' || !/^[0-9a-f]{1,64}$/i.test(obj.requestId)) return false;
  if (typeof obj.challenge !== 'string' || obj.challenge.length < 16 || obj.challenge.length > 512) return false;
  if (typeof obj.origin !== 'string' || obj.origin.length > 1024) return false;
  if (!isValidHttpsUrl(obj.origin)) return false;
  if (typeof obj.timestamp !== 'number') return false;
  if (Math.abs(Date.now() / 1000 - obj.timestamp) > 300) return false;
  if (obj.callbackUrl !== undefined) {
    ...
    if (!isValidHttpsUrl(obj.callbackUrl)) return false;
    try {
      const cbOrigin = new URL(obj.callbackUrl).origin;
      const reqOrigin = new URL(obj.origin).origin;
      if (cbOrigin !== reqOrigin) return false;
    } catch { return false; }
  }
...
/** Check if a URL is wss:// or ws://localhost (dev). */
function isValidWssUrl(url: string): boolean {
...
export function routeQR(data: string): QRAction {
  ...
  if (raw.startsWith('npub1') && raw.length >= 60 && raw.length <= 90 && /^npub1[023456789acdefghjklmnpqrstuvwxyz]+$/.test(raw)) {
    return { type: 'contact', npub: raw };
  }
  if (raw.startsWith('nprofile1') && raw.length <= 500 && /^nprofile1[023456789acdefghjklmnpqrstuvwxyz]+$/.test(raw)) {
    return { type: 'contact', npub: raw };
  }
