...
export function parsePolicy(event: NostrEvent): ParsedPolicy | null {
  ...
  return {
    ...
    adultMinTier: (adultTier ? parseInt(adultTier) : 1) as SignetTier,
    childMinTier: (childTier ? parseInt(childTier) : 1) as SignetTier,
    enforcement: (getTagValue(event, 'enforcement') || 'client') as EnforcementLevel,
    minScore: getTagValue(event, 'min-score') ? parseInt(getTagValue(event, 'min-score')!) : undefined,
    modMinTier: getTagValue(event, 'mod-min-tier') ? parseInt(getTagValue(event, 'mod-min-tier')!) as SignetTier : undefined,
    verifierBond: getTagValue(event, 'verifier-bond') ? parseInt(getTagValue(event, 'verifier-bond')!) : undefined,
    revocationThreshold: getTagValue(event, 'revocation-threshold') ? parseInt(getTagValue(event, 'revocation-threshold')!) : undefined,
  };
}
